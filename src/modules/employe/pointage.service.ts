...
const PointageService = {
  ...
  findByEmploye: async (employeId: string, startDate?: Date, endDate?: Date) => {
  ...
  findByEntreprise: async (entrepriseId: string, startDate?: Date, endDate?: Date) => {
    const params: any = { entrepriseId };
    if (startDate) params.startDate = startDate;
    if (endDate) params.endDate = endDate;
    return PointageModel.findAll(params);
  },
  ...
  findTodayByEmploye: async (employeId: string) => {
    const today = new Date();
  ...
  canCheckIn: async (employeId: string, location?: { lat: number; lng: number }) => {
  ...
  generateQRCode: async (employeId: string, entrepriseId: string) => {
  ...
  generateCompanyQRCode: async (entrepriseId: string) => {
    const timestamp = Date.now();
    const qrData = {
      entrepriseId,
      timestamp,
  ...
  // Valider un QR code
  validateQRCode: async (qrString: string, location?: { lat: number; lng: number }) => {
    try {
  ...
  getStats: async (employeId?: string, startDate?: Date, endDate?: Date) => {
    const params: any = {};
...
const PresenceService = {
  ...
  calculateMonthlyHours: async (employeId: string, year: number, month: number) => {
  ...
  // Marquer une absence
  markAbsence: async (employeId: string, date: Date, reason: string) => {
  ...
  // Marquer un congé
  markLeave: async (employeId: string, date: Date, type: 'conge' | 'maladie' | 'ferie') => {
    return PresenceModel.upsert(employeId, date, {
      statut: type,
      heuresTravaillees: 0
    });
